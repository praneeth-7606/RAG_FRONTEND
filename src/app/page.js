...
export default function Home() {
  ...
  const documentsLoadedNotified = useRef(false)
  ...
  const handleDocumentUploadError = useCallback((error, filename) => {
   ...
   const handleChatResponse = useCallback((query, response, sourceDocs) => {
    const newMessage = {
      id: Date.now(),
      query,
      response,
      sources: sourceDocs || [], // Storing sources with the message is good practice
      timestamp: new Date().toISOString()
    };
    
    setChatHistory(prev => [...prev, newMessage]);

    // This now correctly updates the one and only sources state
    setCurrentSources(sourceDocs || []);
    
    // This notification logic is also correct
    if (sourceDocs && sourceDocs.length > 0) {
      showToast(
        `Found ${sourceDocs.length} relevant source${sourceDocs.length > 1 ? 's' : ''}. Check the Sources tab.`,
        'info',
        4000
      );
    }
