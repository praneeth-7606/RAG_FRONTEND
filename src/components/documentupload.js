...
import { FileTextOutlined, CheckCircleOutlined, CloudUploadOutlined, LoadingOutlined } from '@ant-design/icons'
...
export default function DocumentUpload({ onUploadSuccess, onUploadError, uploadedDocuments }) {
  ...
  const [processingTasks, setProcessingTasks] = useState(new Map()) // Track background tasks
  ...
  const pollTaskStatus = async (taskId, filename, fileSize) => {
    ...
    const poll = async () => {
      try {
        ...
        if (taskData.status === 'processing') {
          const progress = Math.min(taskData.progress || 50, 95)
          setUploadProgress(progress)
          
          // Update processing tasks map
          ...
          // Continue polling if still processing and not timed out
          ...
          setTimeout(() => {
            onUploadSuccess({
              filename: filename,
              documentId: taskData.result?.document_id || taskId,
              chunksCount: taskData.result?.chunks_count || 0,
          ...
          const errorMsg = taskData.message || 'Document processing failed'
        ...
        if (onUploadError) {
          onUploadError(pollError.message, filename)
        } else {
          setError({
    ...
    try {
      ...
      // If your backend needs a specific namespace, you can add it here
      // formData.append('namespace', 'my-custom-namespace');
      ...
      if (!response.ok) {
        // Server responded with an error (e.g., status 400 or 500)
        throw new Error(data.detail || `Server error: ${response.status}`);
      }

      //  SUCCESS!
      // The backend response is successful and contains the data we need.
      setUploadProgress(100);

      // A short delay to let the user see the "100%" state before it disappears
      setTimeout(() => {
        // Call the onUploadSuccess function passed from the parent component
        onUploadSuccess({
          filename: data.filename,           // Get filename from backend response
          chunksCount: data.chunks_uploaded, // Get chunk count from backend response
          uploadTime: new Date().toISOString(),
          fileSize: file.size,
          documentId: data.filename, // Use filename as a simple unique ID
          status: 'completed'
        });
      ...
      // Use the onUploadError function from the parent or the local state
      if (onUploadError) {
    ...
    <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 sm:gap-10">
      ...
      <div className="xl:col-span-7">
        <div className="text-center px-4 sm:px-0">
          ...
          {/*  NEW: Processing Tasks Display */}
          {processingTasks.size > 0 && (
            <div className="mb-6 sm:mb-8">
              <h3 className="text-white font-semibold text-lg mb-4"> Processing Documents</h3>
              {Array.from(processingTasks.entries()).map(([taskId, task]) => (
                <div key={taskId} className="bg-yellow-500 bg-opacity-20 border border-yellow-400 border-opacity-30 text-yellow-100 px-4 py-3 rounded-2xl mb-3">
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-semibold">{task.filename}</span>
                    <span className="text-sm">{task.progress}%</span>
                  </div>
                  <div className="w-full bg-yellow-600 bg-opacity-30 rounded-full h-2 mb-2">
                    <div 
            ...
            <div className="relative z-10">
              ...
              <div className="space-y-4">
                ...
                <p className="text-gray-300 text-sm sm:text-lg">
                  {uploading ? 'AI is analyzing your insurance policy with background processing' : 'Upload PDF or TXT insurance documents for intelligent analysis'}
                </p>
            ...
            <div className="mb-6 sm:mb-10">
              <div className="flex justify-between items-center mb-3 sm:mb-4">
                <span className="text-white font-semibold text-base sm:text-lg">
                  {uploadProgress < 20 ? 'Uploading...' : uploadProgress < 50 ? 'AI Analysis...' : 'Creating Embeddings...'}
                </span>
              ...
              <div className="mt-2 sm:mt-3 text-center">
                <p className="text-gray-400 text-xs sm:text-sm">
                  {uploadProgress < 20 ? 'Uploading file to server...' : 
                   uploadProgress < 50 ? 'AI analyzing document structure and content...' : 
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-2">
                        ...
                        <span className="text-xs text-green-400 font-medium mt-1 block">
                          Processing complete
                        </span>
                      </div>
                    </div>
